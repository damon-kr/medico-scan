...
**목표**: 조건부 질문 필터링 구현 및 진료분야 1순위/2순위 선택 방식 적용

**완료 작업**:
- [x] Q2 진료분야를 ranking 타입으로 변경 (1순위, 2순위 선택)
- [x] Survey.tsx에 조건부 질문 필터링 로직 추가 (getFilteredQuestions)
- [x] 2순위가 '피부과/성형외과'인 경우 1순위로 처리하는 로직 구현
- [x] Q4-1 (상업적 플랫폼 활용) 조건부 표시 로직 구현
- [x] 진료분야 응답 변경 시 조건부 질문 응답 초기화 로직 추가
- [x] 문서 업데이트 (02-domain-logic.md, 03-data-schema.md, 06-progress.md)
