...
export const RankingQuestion = ({ question, value, onChange }: RankingQuestionProps) => {
  ...
  const maxRank = Math.min(question.validation?.max || selected.length, selected.length);
  ...
  const handleToggle = (optionValue: string) => {
    ...
    const newSelected = isCurrentlySelected ? selected.filter((v) => v !== optionValue) : [...selected, optionValue];
    ...
    if (isCurrentlySelected) {
      ...
      const newMaxRank = Math.min(question.validation?.max || newSelected.length, newSelected.length);
  ...
  const getAvailableRanks = (currentOptionValue: string) => {
    ...
    return Array.from({ length: maxRank }, (_, i) => i + 1).filter((rank) => !usedRanks.includes(rank));
  };
